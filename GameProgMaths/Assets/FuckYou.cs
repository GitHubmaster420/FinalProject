...
public class BezierPath : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        int n = points.Length;
        int nSegments = n - 1;
        int seg_start = Mathf.FloorToInt(TSimulate * nSegments);
        float TActual = (float)nSegments * TSimulate - seg_start * 1.0f;
        ...
        MyObject.transform.position = op.pos;
        MyObject.transform.rotation = op.rot;
        TSimulate += Time.deltaTime * 0.1f;
...
        */
    ...
                */
                ...
                for (int i = 0; i < Roadshape.vertices.Length; i++)
                {
                    ...
                    Vector3 transformed_point = op.LocalToWorldPos(roadpoint * RoadScaler);
                    vertices.Add(transformed_point);



                    Vector3 transformed_point_next = op.LocalToWorldPos(roadpoint_next * RoadScaler);



                    Vector3 transformed_point_following = op_next.LocalToWorldPos(roadpoint * RoadScaler);
    ...
                */
                ...
                for (int i = 0; i < Roadshape.vertices.Length; i++)
                {
                    ...
                    uvs.Add(new Vector2((roadpoint.x + 5) / 10, TActual));
